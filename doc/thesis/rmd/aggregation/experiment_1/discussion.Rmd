...
We found that choice bracketing facilitated risk aggregation in
...
In addition to the novelty of the paradigm itself, we found that choice
bracketing facilitates risk aggregation. As per Hypothesis 1, we found that
merely knowing that further choices will have to be made reduces risk aversion.
As per Hypothesis 2, we found that people are less risk averse when viewing
their choices together. Contrary to Hypothesis 3, we did not find a main effect
...
easily compared. Unsurprisingly, as per Hypothesis 4, we found that risk
...
encourage risk aggregation, people still struggled to aggregate as well as when
...
amount of risky choices they made. We found that risky choice increased with an
...
options as fungible [e.g., @dekay2005]. As such, we predicted that a set of
investments that involve the same type of investment would be seen as more
similar and fungible. We expected that this would facilitate a broad bracketing,
and therefore more risk aggregation.

While we did not find a main effect of similarity, we found that choice
similarity interacted with the presentation condition. We are unable to clearly
account for this effect, since we would have expected to see it in the opposite
direction. That is, we would have expected that seeing a set of similar projects
...
repeated-play paradigms [e.g., @benartzi1999]. As we mentioned previously, both
...
feedback. We are the first to provide evidence for this aggregation effect with
...
One potential limitation of our experiment is that we provided participants with
...
difference in the effects we found in a real-life ambiguous choice environment.
