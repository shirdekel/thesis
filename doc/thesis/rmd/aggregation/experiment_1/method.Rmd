...
projects, and asked whether they would invest in that project or not. Each
...
probabilities for these forecasts. We constructed these projects to appear
...
[^5]: The actual probability of a loss given the outcome distribution we used
...
type of projects and were all from the same business. In the high similarity
condition, each project was from a different industry. In the joint presentation
condition, the 10 projects were all displayed on the one webpage, whereas in the
separate presentation condition each was displayed on a different webpage.
Participants in the aware condition saw the display shown in
Figure\ \@ref(fig:awareness-aware-aggregation-1) before their separate presentation
display. Those in the naive condition simply proceeded without this message.
Note, the financial and probability values were identical regardless of
condition, and the order of each set of 10 projects was randomised.
...
We wanted to make sure that participants were generally making decisions that
were in line with EV theory and that our sample was not abnormally risk
tolerant. As such, we presented participants two project decisions that had a
...
Participants then made the outcome distribution decision and subsequently
completed the negative EV gambles.
