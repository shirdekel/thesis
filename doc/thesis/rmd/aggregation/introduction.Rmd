# Effects of choice bracketing on risk aggregation in repeated-play gambles with no feedback

## Introduction

Investors know not to put all their eggs in one basket, but do the rest of us?
Ever since work on modern portfolio theory [@markowitz1952], it has been clear
that combining the risk of a set of individual investments reduces the overall
risk of the portfolio of investments. But what about situations in which it is
not clear that a set of investments fit together as a portfolio? Personal
decisions such as buying a car or moving cities are typically evaluated
independently, as are business decisions such as a farm investing in new
cropping technology or an multi-business firm building a mine.
...
in the expected return on investment. These are the gambles that I investigate
in the current study: large risky choices without immediate outcomes.

Work on prospect theory [@kahneman1979] suggests that people's evaluation of
gambles does not conform to expected utility theory and is prone to framing
effects. Specifically, people typically evaluate gambles one by one [@rabin2009;
...
primary contributions. First, I am the first to investigate the effect of
choice bracketing on risk aggregation in independent gambles evaluated without
...
participants experience individual outcomes from the eventual outcome
distribution of the gambles, meaning that aggregation is confounded with
learning. As mentioned above, in real-life there is usually a significant delay
between the choice a person makes and the outcome of that choice, and there are
...
choice bracketing on description-based repeated-play gambles. This is
...
information more comprehensively [@bjornsen2019]. In financial settings, the
benefits of investment diversification are primarily due to risk aggregation.
Among 10 risky gambles, one paying off might offset the other nine failing,
especially if the investments are uncorrelated or negatively correlated. The
advantages of diversification, through risk aggregation, is well-known in
investment settings because of modern portfolio theory [@markowitz1952].
...
Despite the difficulties of risk aggregation, people seem to aggregate naturally
when considering multiple gambles. @samuelson1963 told of a colleague that
rejected a gamble that involved a 50% chance of gaining \$200 and a 50% of
losing \$100. However, the same colleague said he would accept 100 plays of the
same gamble.[^1] That is, the colleague was more risk averse when making a
single decision about one gamble (a *single-play* gamble), than when making a
single decision about multiple identical gambles (a *multi-play* gamble).[^2]

[^1]: @samuelson1963 argued that this choice is irrational, but other work
    suggests that it is consistent with expected utility theory with [e.g.,
    @ross1999; @lopes1981; @aloysius2007]. However, a normative discussion is
    out of the scope of the present article.

[^2]: I use the terminology from @bristow2011 and @camilleri2013 for
    consistency.
...
Some participants often require fewer than 10 plays of a previously rejected
gamble in order to accept it [@dekay2005; @keren1991; @montgomery1982;
@redelmeier1992].[^3] Multi-play gambles reduce a variety of cognitive biases ,
such as common-ratio effects [@keren1987; @keren1991; @dekay2006], preference
reversals [@wedell1990], ambiguity aversion [@liu2009], and the illusion of
control [@koehler1994]. Participants are also more likely to use explicitly
provided Expected Values (EVs) in multi-play gambles [@li2003], show eye
movements more congruent with an EV model than single-play gambles [@su2013],
and judge multi-play gambles as riskier [@joag1990].

[^3]: Other similar studies found a multi-play effect that was in the predicted
    direction but not significant [@barron2003; @benartzi1999; @klos2005;
    @langer2001]. Further, the effect is not seen when participants do not
    perceive gamble outcomes as fungible [@dekay2006; @dekay2005; @dekay2011] or
    when choice is continuous rather than discrete [@bristow2011].

Multi-play gambles that are displayed with an aggregated outcome distribution of
...
@coombs1971; @venkatraman2006; @dekay2005; @langer2001; @keren1991].[^4] The
multi-play gamble is usually set up such that its (binomial) outcome
distribution shows a relatively low chance of losing any money and a very low
chance of losing a lot of money. As such, the displayed outcome distributions do
the aggregating work for the participants, making the attractiveness of the
multi-play gamble explicit. This work suggests that participants can comprehend
and respond to aggregated risk, but that they struggle to compute the
aggregation without external help.

[^4]: This does not seems to hold when returns are calculated as percentages,
    rather than fixed dollar amounts [@stutzer2013]; and when participants do
    not perceive gamble outcomes as fungible [@dekay2005].
...
@ludvig2011; @hertwig2004; @jessup2008]. Other work with a repeated-play
paradigm found that participants showed less risk aversion when being instructed
to consider the options as a part of a portfolio [@sokolhessner2009;
@sokolhessner2012].
...
repeated-play gamble literature uses various forms of feedback. That is,
participants are shown the outcomes of their gambles at multiple points in the
experiment. This paradigm is known as *experience-based choice*. In
*description-based choice* on the other hand, the gamble is simply presented to
the participant without any feedback, as in the multi-play gambles above. In
real life, people rarely see the immediate outcomes of their risky choices, and even
less so in financial settings, where any return on investment often takes years
to manifest.

Only a limited number of studies used a repeated-play paradigm without feedback.
For instance, @jessup2008 and @hertwig2004 investigated the effects of feedback
in repeated-play gambles on the weighting of small probabilities, so therefore
...
aggregation by encouraging people choice grouping. Specifically, people
...
The goal of this study is to facilitate risk aggregation without the
...
them individually -- reduced risk aversion. However, I am interested in how
more subtle cues may facilitate risk aggregation, without relying on explicit
education. The previous literature suggests that lay people do not know to
aggregate risk, but this research did not directly test contextual cues that
would encourage choice bracketing. It is quite possible that without explicit
instruction, supporting choice bracketing will "naturally" lead to risk
aggregation and thus reduce risk aversion. For example, simply making
participants aware that they are going to be making a series of choices may be
sufficient to encourage risk aggregation. Of course, the effects of more subtle
cues would not eliminate the utility of explicit financial education, but they
will help the design of decision-making contexts to best align with such
instruction. In addition, the effects of more subtle cues help shed light on the
cognitive processes underlying choice bracketing.
...
for example, a technology business vs. an oil business.
...
people's choices are affected by whether they view their the attributes of the
...
It is unclear how much something such as the gambler's fallacy could affect
repeated-play tasks. @barron2010 found that the gambler's fallacy (in a roulette
prediction task) emerges when information about past outcomes were displayed
sequentially, but not when they are displayed all at once. @haisley2008 found
evidence for the gambler's fallacy with a repeated-play gamble paradigm.
However, it is unclear to what extent this is an explanation of repeated-play
gamble effects that do not use feedback.

### Internal market capital investment context

Risk aggregation is salient in the investment world. Much of the work of
investment managers involves constructing investment portfolios that are
adequately diversified, such that the risk is spread throughout the individual
investments. This work primarily takes place in external markets, but do
managers of companies follow similar principles when investing in business
projects within their companies? Do managers of internal market investment
diversify as well as managers of external market investment?

@lovallo2020 suggest that companies do not make as many risky choices with their
internal investment decisions as they should be making. In multi-business firms,
the managers of each business unit often make the investment decisions about
individual projects. As such, they often do not consider the scope of their
decisions in the context of the entire company. For instance, Nobel laureate
Richard Thaler offered 25 division managers working for the same firm a
hypothetical investment that involves a 50% chance of gaining \$2 million for
the company and a 50% chance of losing \$1 million. Only three managers said
they would accept the investment [@thaler1999]. However, the CEO indicated that
he would have clearly preferred all the managers to accept the investments. To
each middle-manager, the choice represents a risk of loss for their division and
potentially their job, whereas for the CEO the entire portfolio of choices
represents a likely overall gain for the company.

In this chapter I investigate risky choice in the context of business
project investment internal to a company because this is a real-world context
where choice bracketing is important and currently under-utilised [@lovallo2020].
The participants in my studies are taken from a general
population that does not have extensive managerial experience. However, in the
general population a lack of risk aggregation is most likely more common, and
the variables I am researching here are readily applicable to the financial
decisions that lay people make. For instance, one of the real-world applications
of the choice bracketing literature has been to use outcome distributions and
increased time horizons to encourage investment in high risk, but high EV, funds
...
decision-context. Future research will focus on participants with
