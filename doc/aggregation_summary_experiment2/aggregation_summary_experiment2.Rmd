...
loadd(descriptives_experiment2)
loadd(results_experiment2)
loadd(plot_experiment2)
```

# Participants

`r descriptives_experiment2$apa` Table\ \@ref(tab:allocation) shows the condition allocation.

```{r allocation}
descriptives_experiment2$allocation %>% 
...
Figures\ \@ref(fig:plot-choice-binary) and\ \@ref(fig:plot-choice-proportion) show the choice and proportion data, respectively. The difference between presentation conditions was not significant, both in the logistic regression `r results_experiment2$choice$presentation$full_result[[2]]`, and in the t-test, `r results_experiment2$proportion$presentation`. Similarly, the difference between awareness conditions was not significant, both in the logistic regression `r results_experiment2$choice$awareness$full_result[[2]]`, and in the t-test, `r results_experiment2$proportion$awareness`. However, those that that saw a distribution tended to choose to invest significantly more than those that did not see a distribution, seen both in the logistic regression, `r results_experiment2$choice$distribution$full_result[[2]]`, and the t-test `r results_experiment2$proportion$distribution`.
...
plot_experiment2$choice_binary
```
...
plot_experiment2$choice_proportion
```
...
plot_experiment2$choice_trials
```
...
plot_experiment2$project_number
```

### Portfolio choice - binary

Participants were then asked if they would rather invest in all or none of the projects. As Figure\ \@ref(fig:plot-portfolio-binary) shows, the difference between presentation conditions was not significant, `r results_experiment2$portfolio_binary$presentation$full_result[[2]]`. The awareness effect was also not significant, `r results_experiment2$portfolio_binary$awareness$full_result[[2]]`. However, those that that saw a distribution chose to invest in all 10 projects significantly more than those that did not see a distribution, `r results_experiment2$portfolio_binary$distribution$full_result[[2]]`. 
...
plot_experiment2$portfolio_binary
```

### Portfolio choice - number

Subsequently, we asked participants how many projects they would invest in out of the 10 that they saw. As Figure\ \@ref(fig:plot-portfolio-number) shows, the difference between presentation conditions was not significant, `r results_experiment2$portfolio_number$presentation`. The awareness effect was also not significant, `r results_experiment2$portfolio_number$awareness`. However, those that that saw a distribution chose to invest in significantly more projects than those that did not see a distribution, `r results_experiment2$portfolio_number$distribution`. 
...
plot_experiment2$portfolio_number
```
...
plot_experiment2$gamble_values
```
