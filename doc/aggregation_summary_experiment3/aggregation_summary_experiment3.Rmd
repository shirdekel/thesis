...
loadd(descriptives_experiment3)
loadd(results_experiment3)
loadd(plot_experiment3)
```

# Participants

`r descriptives_experiment3$apa` Table\ \@ref(tab:allocation) shows the condition allocation.

```{r allocation}
descriptives_experiment3$allocation %>% 
...
Figures\ \@ref(fig:plot-choice-binary) and\ \@ref(fig:plot-choice-proportion) show the choice and proportion data, respectively. The difference between similarity conditions was not significant, both in the logistic regression `r results_experiment3$choice$full_result[[2]]`, and in the t-test, `r results_experiment3$proportion`.

```{r plot-choice-binary, fig.cap = "Mean project acceptance for the similarity effect."}
plot_experiment3$choice_binary
```

```{r plot-choice-proportion, fig.cap = "Mean proportion of project acceptance for the similarity effect."}
plot_experiment3$choice_proportion
```
...
plot_experiment3$choice_trials
```
...
plot_experiment3$project_expectation
```
...
plot_experiment3$project_number
```

### Portfolio choice - binary

Participants were then asked if they would rather invest in all or none of the projects. As Figure\ \@ref(fig:plot-portfolio-binary) shows, the difference between presentation conditions was not significant, `r results_experiment3$portfolio_binary$full_result[[2]]`. 

```{r plot-portfolio-binary, fig.cap = "Mean choice of investing in all 10 projects for the similarity effect."}
plot_experiment3$portfolio_binary
```

### Portfolio choice - number

Subsequently, we asked participants how many projects they would invest in out of the 10 that they saw. As Figure\ \@ref(fig:plot-portfolio-number) shows, the difference between presentation conditions was not significant, `r results_experiment3$portfolio_number`.

```{r plot-portfolio-number, fig.cap = "Mean number of projects chosen in the follow-up for the similarity effect."}
plot_experiment3$portfolio_number
```
...
plot_experiment3$gamble_values
```
